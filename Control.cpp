...
{
	if (ClientState() != ClientStateMenu) return NULL;
...
Control* findControl(int Type, const wchar_t* Text, int Disabled, int PosX, int PosY, int SizeX, int SizeY)
{
	...
	for (Control* pControl = *p_D2WIN_FirstControl; pControl; pControl = pControl->pNext)
	{
		...
		{
			if (!pControl->wText2) return NULL;
		...
		if (Text && pControl->dwType == CONTROL_TEXTBOX)
		{
			...
			{
				if (!pControl->pFirstText->wText[0]) return NULL;
...
bool clickControl(Control* pControl, int x, int y)
{
	...
	if (pControl)
	{
		...
		if (y == -1) y = pControl->dwPosY - (pControl->dwSizeY / 2);
...
{
	if (OOG_GetLocation() != OOG_CHAR_SELECT || wcslen(szCharacter) > 15 || type > 6 || type < 0) return FALSE;

	// click the create character button
	Control* ctrl = findControl(CONTROL_BUTTON, (const wchar_t*)NULL, -1, 33, 528, 168, 60);
	if (ctrl) clickControl(ctrl);
	...
	if (_wcsicmp(name, ctrl->pFirstText->wText[0]) != 0) return FALSE; // something bad happened?
...
{
	if (ClientState() != ClientStateMenu) return;
...
BOOL OOG_SelectGateway(const wchar_t* szGateway, size_t strSize)
{
	...
	if (wcsstr(szGateway, L"ERROR")) return FALSE;
...
{
	if (ClientState() != ClientStateMenu) return OOG_NONE;
	...
	{
		if (findControl(CONTROL_BUTTON, 5288, -1, 264, 484, 272, 35)) return OOG_LOGIN; // 9 Login
		if (findControl(CONTROL_BUTTON, 5102, -1, 495, 438, 96, 32))
			return OOG_CHARACTER_SELECT_CHANGE_REALM; // 43 char select change realm
		if (findControl(CONTROL_BUTTON, 5102, -1, 627, 572, 128, 35) &&
			findControl(CONTROL_BUTTON, 10832, -1, 33, 528, 168, 60)) // 10832=create new
		{
			...
			{
				if (findControl(CONTROL_TEXTBOX, 11162, -1, 45, 318, 531, 140) ||
					findControl(CONTROL_TEXTBOX, 11066, -1, 45, 318, 531, 140))
...
bool OOG_CreateGame(const wchar_t* name, const wchar_t* pass, int difficulty)
{
	...
	if (!name || !pass || wcslen(name) > 15 || wcslen(pass) > 15) return FALSE;

	Control* pControl = NULL;

	// Battle.net/open game creation
	OOG_Location loc = OOG_GetLocation();
	if (!(loc == OOG_LOBBY || loc == OOG_CHAT || loc == OOG_DIFFICULTY || loc == OOG_CREATE)) return FALSE;
	...
	if (loc == OOG_DIFFICULTY)
	{
		...
		switch (difficulty)
		{
			case 0: // normal button
				if (normal->dwDisabled != 0x0d || !clickControl(normal)) return FALSE;
				break;
			case 1: // nightmare button
				if (nightmare->dwDisabled != 0x0d || !clickControl(nightmare)) return FALSE;
				break;
			case 2: // hell button
				if (hell->dwDisabled != 0x0d || !clickControl(hell)) return FALSE;
				break;
			case 3: // hardest difficulty available
				if (hell->dwDisabled != 0x0d)
				{
					if (!clickControl(hell)) return FALSE;
				}
				else if (nightmare->dwDisabled != 0x0d)
				{
					if (!clickControl(nightmare)) return FALSE;
				}
				else if (normal->dwDisabled != 0x0d)
				{
					if (!clickControl(normal)) return FALSE;
				}
				break;
			default:
				return FALSE;
	...
	else
	{
		...
		if (loc != OOG_CREATE)
		{
			...
			if (!pControl || !clickControl(pControl)) return FALSE;
			Sleep(100);
		}
		if (OOG_GetLocation() == OOG_CREATE)
		{
			...
			{
				case 0: // normal button
					if (normal->dwDisabled == 0x4 || !clickControl(normal)) return FALSE;
					break;
				case 1: // nightmare button
					if (nightmare->dwDisabled == 0x4 || !clickControl(nightmare)) return FALSE;
					break;
				case 2: // hell button
					if (hell->dwDisabled == 0x4 || !clickControl(hell)) return FALSE;
					break;
				case 3: // hardest difficulty available
...
bool OOG_JoinGame(const wchar_t* name, const wchar_t* pass)
{
	...
	// reject name/password combinations over 15 characters
	if (wcslen(name) > 15 || wcslen(pass) > 15) return FALSE;
