...
{
	return (
		ClientState() == ClientStateInGame ? D2CLIENT_GetPlayerUnit()->pPath->pRoom1->pRoom2->pLevel->dwLevelNo : NULL);
}
...
void D2CLIENT_Interact(UnitAny* pUnit, DWORD dwMoveType)
{
	...
	if (!D2CLIENT_FindUnit(pUnit->dwUnitId, pUnit->dwType)) return;

	UnitInteraction pInteract = {
		dwMoveType, D2CLIENT_GetPlayerUnit(), pUnit, D2CLIENT_GetUnitX(pUnit), D2CLIENT_GetUnitY(pUnit), 0, 0};
...
void __declspec(naked) __fastcall D2CLIENT_ShopAction_ASM(
	...
	DWORD pNPC,
	DWORD _1,
	DWORD pTable2 /* Could be also the ItemCost?*/,
	DWORD dwMode,
	DWORD _2,
	DWORD _3)
{
...
void __declspec(naked) __stdcall D2CLIENT_LeftClickItem(
	DWORD Location, UnitAny* pPlayer, Inventory* pInventory, int x, int y, DWORD dwClickType, InventoryLayout* pLayout)
{
	__asm
	{
...
void __declspec(naked) __fastcall D2CLIENT_ClickItemRight_ASM(
	DWORD x, DWORD y, DWORD Location, DWORD Player, DWORD pUnitInventory)
{
	__asm
	{
...
void __declspec(naked) __fastcall D2CLIENT_ClickBeltRight_ASM(
	DWORD pInventory, DWORD pPlayer, DWORD HoldShift, DWORD dwPotPos)
{
	__asm
	{
		pop eax // pop return address
...
bool IsScrollingText()
{
	...
	WindowHandlerList* whl =
		p_STORM_WindowHandlers->table[(0x534D5347 ^ (DWORD)d2Hwnd) % p_STORM_WindowHandlers->length];
