...
int CArrayEx<TYPE, ARG_TYPE>::_CompareAscending(const void* p1, const void* p2)
{
	...
	if (*pElement1 == *pElement2) return 0;

	if (*pElement1 > *pElement2) return 1;
...
{
	if (m_nElementCount < 2 || (bAscending && (m_nSort == AE_SORT_ASCENDING)) ||
...
{
	if (nStartAt < 0) nStartAt = 0;

	if (!IsIndexValid(nStartAt)) return -1;
...
BOOL CArrayEx<TYPE, ARG_TYPE>::SetSize(int nNewSize)
{
	...
	if (!_GrowSize(nNewSize - m_nElementCount)) return FALSE;
...
int CArrayEx<TYPE, ARG_TYPE>::InsertAt(int nIndex, ARG_TYPE tData, int nCount)
{
	...
	if (!_MakeSpaces(nIndex, nCount)) return -1;
...
{
	if (pNewArray == NULL || pNewArray->GetSize() < 1) return -1;

	_AdjustIdx(nIndex, TRUE);
	if (!_MakeSpaces(nIndex, pNewArray->GetSize())) return -1;
...
{
	if (nCount > m_nElementCount - nIndex) nCount = m_nElementCount - nIndex;

	if (!IsIndexValid(nIndex) || nCount < 1) return 0;
...
{
	if (m_pData == NULL || m_nAllocSize == m_nElementCount) return TRUE;
...
{
	if (!SetSize(src.m_nElementCount)) return FALSE;
...
int CArrayEx<TYPE, ARG_TYPE>::Append(const CArrayEx& src)
{
	...
	if (!_GrowSize(src.m_nElementCount)) return -1;
...
void CArrayEx<TYPE, ARG_TYPE>::_AdjustIdx(int& nIndex, BOOL bAllowGrow) const
{
	...
	if (nIndex >= m_nElementCount) nIndex = bAllowGrow ? m_nElementCount : (m_nElementCount - 1);
}
...
BOOL CArrayEx<TYPE, ARG_TYPE>::_MakeSpaces(int nIndex, int nCount /*= 1*/)
{
	...
	if (!_GrowSize(nCount)) return FALSE;
...
{
	if (!IsIndexValid(nIndex)) return FALSE;
