...
public:
	...
	void GetOpenNodes(Point const& center, PointList& out, Point const& endpoint)
	{
		...
		// map->pathinging point further restricsts the nodes. for tele path we dont care if its the best possible path.
		// cuts time in half if were in tele range take the jump
		...
		if (bestPtSoFar.first == 0) bestPtSoFar = center;
		...
		if (Euclidean(bestPtSoFar, center) < 500)
		{
				...
				Euclidean(best, endpoint) < Euclidean(center, endpoint))
			{
				...
				out.push_back(best);
				if (Euclidean(best, endpoint) < Euclidean(bestPtSoFar, endpoint)) bestPtSoFar = best;
				return;
			}
		}
		justExpand = true;
		...
		for (int i = 1; i >= -1; i--)
		{
			for (int j = 1; j >= -1; j--)
			{
				if (i == 0 && j == 0 || Reject(Point(center.first + i, center.second + j), true)) continue;
		...
		for(int i = range/10; i >= range/10*-1; i =i-4)
		{
				...
				if(distance(Point(center.first+i, center.second+j), center) < range)
						if(map->PathingPointList.find(Point(center.first+i, center.second+j)) ==
		map->PathingPointList.end())
						{
								out.push_back(Point(center.first+i, center.second+j));
								map->PathingPointList.insert(Point(center.first+i, center.second+j));
						}
