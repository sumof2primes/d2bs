	...
	*/
	...
	void Reduce(PointList const& in, PointList& out, bool abs)
	{
		...
		while (it != end)
		{
			...
			if (lineLength > 5 && ((currentStep != lineStartStep && currentStep != secondStep) ||
								   differentStepCount > 1 || lineLength > 20))
			{
				out.push_back(previousPoint);
		...
		sprintf_s(mapFileName, "b:\\maps-pathing\\%03d %s in.txt", map->GetLevel()->dwLevelNo,
		GetLevelName(map->GetLevel())); map->Dump(mapFileName, in); sprintf_s(mapFileName, "b:\\maps-pathing\\%03d %s
		out.txt", map->GetLevel()->dwLevelNo, GetLevelName(map->GetLevel())); map->Dump(mapFileName, out);
		*/
	}
	...
	int GetPenalty(Point const& pt, bool abs)
	{
		...
		return 0;
		// return map->SpaceHasFlagWide(ActMap::BlockWalk, pt, abs) ? 50 : (data & ActMap::Object == ActMap::Object) ?
		// 60 : (map->SpaceHasFlag(ActMap::ClosedDoor, pt, abs) ? 80 : 0));
	}
	...
	void MutatePoint(Point& pt, bool absv)
	{
		...
		for (int i = -3; i <= 3; i++)
		{
			for (int j = -3; j <= 3; j++)
			{
				if (i == 0 && j == 0 || (abs(i) + abs(j)) == 6) continue;
		...
		for (int i = -2; i <= 2; i++)
		{
			for (int j = -2; j <= 2; j++)
			{
				if (i == 0 && j == 0 || (abs(i + j)) == 1) continue;

				if (!checkFlag(
						area[3 + i][3 + j] | area[3 + i + 1][3 + j] | area[3 + i - 1][3 + j] | area[3 + i][3 + j + 1] |
						area[3 + i][3 + j - 1]))
				{
					pt.first += i;
					pt.second += j;
