...
ActMap::ActMap(const Level* level)
{
	...
	if (!level->pRoom2First) D2COMMON_InitLevel(const_cast<Level*>(level));
...
{
	if (!abs) return GetMapData(RelativeToAbs(point), true);

	// check in rooms we already read
	for (RoomList::iterator cRoom = roomCache.begin(); cRoom != roomCache.end(); cRoom++)
		if (isPointInRoom(*cRoom, point)) return getCollFromRoom(*cRoom, point);
	...
	if (!currLevel)
	{
		...
		{
			if (isPointInLevel(*cLvl, point)) currLevel = *cLvl;
		}
	}
	...
	if (!currLevel)
	{
		for (Level* lvl = this->act->pMisc->pLevelFirst; lvl; lvl = lvl->pNextLevel)
		{
			...
			{
				if (!lvl->pRoom2First) D2COMMON_InitLevel(lvl);
...
bool ActMap::isPointInRoom(const Room2* room, const Point& pt) const
{
	...
	return (
		nX >= room->dwPosX * 5 && nY >= room->dwPosY * 5 && nX < (room->dwPosX * 5 + room->dwSizeX * 5) &&
		nY < (room->dwPosY * 5 + room->dwSizeY * 5));
}
...
bool ActMap::isPointInLevel(const Level* level, const Point& pt) const
{
	...
	DWORD y = pt.second;
	return (
		x >= level->dwPosX * 5 && y >= level->dwPosY * 5 && x < (level->dwPosX * 5 + level->dwSizeX * 5) &&
		y < (level->dwPosY * 5 + level->dwSizeY * 5));
}
...
WORD ActMap::getCollFromRoom(Room2* room, const Point& pt) const
{
	...
	// but for now, just try it with barricade wall in Frigid Highlands (111), Arreat Plateau (112) and Frozen Tundra
	// (117)
	if (room->pLevel->dwLevelNo == 111 || room->pLevel->dwLevelNo == 112 || room->pLevel->dwLevelNo == 117 ||
		room->pLevel->dwLevelNo == 74)
	{
...
void ActMap::GetExits(ExitArray& exits) const
{
	...
	for (Room2* room = level->pRoom2First; room; room = room->pRoom2Next)
	{
		...
		if (room->pLevel->dwLevelNo != level->dwLevelNo) continue;
...
void ActMap::FindRoomLinkageExits(ExitArray& exits) const
{
	...
	for (Room2* room = level->pRoom2First; room; room = room->pRoom2Next)
	{
		...
		{
			if (rooms[i]->pLevel->dwLevelNo == level->dwLevelNo) continue;
			...
			// A -> origin of local room, B -> opposite corner from origin in local room ... X, Y -> like local but for
			// adjecent room
			...
			{
				// Print("пїЅc1d2bsпїЅc3ActMap::GetExitsпїЅc0 adjecent room is out of reach - it's not even touching
				// local room in corner!");
				continue;
			}
			if (overlappingX > 0 && overlappingY > 0)
			{
				// Print("пїЅc1d2bsпїЅc3ActMap::GetExitsпїЅc0 WTF local and adjecent rooms are overlapping (they share
				// some points)!!!");
				continue;
			}
			...
			for (j = 0; j < edgeSize; j++)
			{
				...
				currentPoint =
					Point(startPoint.first + j * edgeDirection.first, startPoint.second + j * edgeDirection.second);
				...
				if (false == isPointWalkable || j + 1 == edgeSize)
				{
					if (spaces > 0)
					{
						...
						currentPoint = Point(
							lastWalkablePoint.first - edgeDirection.first * spaces / 2,
							lastWalkablePoint.second - edgeDirection.second * spaces / 2);
						exitMap.insert(make_pair(
							rooms[i]->pLevel->dwLevelNo, make_pair(centerEdgePoint, make_pair(currentPoint, spaces))));
					}
					spaces = 0;
				}
			}
...
Point ActMap::GetEdgeCenterPoint(const Point& currentPoint, const Point& edgeDirection) const
{
	...
	for (int i = -1; IsValidPoint(startPoint, true); i--)
	{
		...
		startPoint =
			Point(currentPoint.first + edgeDirection.first * i, currentPoint.second + edgeDirection.second * i);
	}
	...
	for (k = distanceLocal; k <= distanceAdjecent; k++)
	{
		if (k <= 0) // simulation of character-size (3 point in width/height) staying in local room
		{
			const Point currentPoint = Point(
				edgePoint.first + offsetPoint.first * (distanceLocal - k),
				edgePoint.second + offsetPoint.second * (distanceLocal - k));
		...
		else // simulation of character-size (3 point in width/height) staying in adjecent room
		{
			const Point currentPoint =
				Point(edgePoint.first + offsetPoint.first * k, edgePoint.second + offsetPoint.second * k);
			if (false == SpaceIsWalkableForExit(currentPoint, abs))
			{
				break;
			}
...
{
	if (!abs) return OneSpaceHasFlag(flag, RelativeToAbs(point), true);
...
{
	if (!abs) return SpaceGetData(RelativeToAbs(point), true);

	int val = GetMapData(point, abs) | GetMapData(Point(point.first - 1, point.second), abs) |
...
{
	if (!abs) return SpaceHasFlag(flag, RelativeToAbs(point), true);
...
{
	if (!abs) return SpaceGetDataWide(RelativeToAbs(point), true);
...
bool ActMap::PathHasFlag(int flag, const PointList& points, bool abs) const
{
	...
	for (int i = 0; i < count; i++)
		if (!SpaceHasFlag(flag, points[i], abs)) return false;
	return true;
}
...
bool ActMap::SpaceIsWalkable(const Point& point, bool abs) const
{
	...
	return !(
		SpaceHasFlag(ActMap::Avoid, point, abs) || SpaceHasFlag(ActMap::BlockWalk, point, abs) ||
		SpaceHasFlag(ActMap::BlockPlayer, point, abs) || SpaceHasFlag(ActMap::NPCCollision, point, abs) ||
...
bool ActMap::PathIsWalkable(const PointList& points, bool abs) const
{
	int count = points.size();
	for (int i = 0; i < count; i++)
		if (!SpaceIsWalkable(points[i], abs)) return false;
	return true;
}
...
bool ActMap::PathHasLineOfSight(const PointList& points, bool abs) const
{
	int count = points.size();
	for (int i = 0; i < count; i++)
		if (!SpaceHasLineOfSight(points[i], abs)) return false;
	return true;
}
...
void ActMap::Dump(const char* file, const PointList& points) const
{
	...
	fprintf(
		f,
		"Map (%d x %d) at (%d, %d) for area %s (id %d):\n",
		width,
		height,
		posX,
		posY,
	...
	for (int j = 0; j < width; j++)
	{
		for (int i = 0; i < height; i++)
		{
			...
			Point pt(i, j);
			char c = '.';
			if (OneSpaceHasFlag(ActMap::ClosedDoor, pt, false)) c = 'D';
			if (OneSpaceHasFlag(ActMap::Object, pt, false)) c = 'O';
			if (OneSpaceHasFlag(ActMap::NPCCollision, pt, false)) c = 'N';
			if (OneSpaceHasFlag(ActMap::BlockPlayer, pt, false)) c = 'P';
			if (OneSpaceHasFlag(ActMap::BlockWalk, pt, false)) c = 'W';
			if (OneSpaceHasFlag(ActMap::Avoid, pt, false)) c = 'A';
			if (pathMap.find(RelativeToAbs(pt)) != pathMap.end()) c = pathMap.find(RelativeToAbs(pt))->second;
			fprintf(f, "%c", c);
		}
		fprintf(f, "\n");
	}
...
void ActMap::DumpLevel(const char* file) const
{
	...
	for (Room2* room = level->pRoom2First; room; room = room->pRoom2Next)
	{
		for (PresetUnit* preset = room->pPreset; preset; preset = preset->pPresetNext)
		{
			...
			fprintf(
				f,
				"Preset %d: at (%d, %d, relative: %d, %d) of type %d, txtFile %d, _1 %d, _3 %d, room (%d, %d) %s\n",
				i++,
				loc.first,
				loc.second,
				rel.first,
	...
	for (Room2* room = level->pRoom2First; room; room = room->pRoom2Next)
	{
		...
		for (UnitAny* pUnit = room->pRoom1->pUnitFirst; pUnit; pUnit = pUnit->pRoomNext)
		{
			...
			fprintf(
				f,
				"Unit %d: at (%d, %d, relative: %d, %d) of type %d, txtFile %d, %s, %d, %d\n",
	...
	for (ExitArray::iterator it = exits.begin(); it != exits.end(); it++)
	{
		...
		fprintf(
			f,
			"Exit %d: at (%d, %d, relative: %d, %d) of type %s linkage to area %s (id: %d, tile id: %d)\n",
			i++,
			it->Position.first,
			it->Position.second,
			rel.first,
			rel.second,
			it->Type == Linkage ? "Area" : "Tile",
			GetLevelIdName(it->Target),
			it->Target,
			it->TileId);
	...
	for (int j = 0; j < width; j++)
	{
		for (int i = 0; i < height; i++)
		{
			...
			if (OneSpaceHasFlag(ActMap::BlockWalk, pt, false)) c = 'W';
			if (OneSpaceHasFlag(ActMap::Avoid, pt, false)) c = 'A';
			if (exitPoints.end() != exitPoints.find(pt)) c = '%';
			fprintf(f, "%c", c);
		}
		fprintf(f, "\n");
	}
