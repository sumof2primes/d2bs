...
{
	if (!room || room->pLevel->dwLevelNo != level->dwLevelNo) return;

	// ignore rooms we've seen before
	if (std::find(rooms.begin(), rooms.end(), room) != rooms.end()) return;
...
void LevelMap::AddCollisionMap(Room1* pRoom1)
{
	...
	int x = map->dwPosGameX - posX, y = map->dwPosGameY - posY, height = map->dwPosGameX + map->dwSizeGameX - posX,
		...
		for (int j = x; j < height && p != map->pMapEnd; j++, p++)
		{
			if ((*p & LevelMap::ClosedDoor) == LevelMap::ClosedDoor || (*p & LevelMap::Player) == LevelMap::Player)
			{
				...
				((*p & LevelMap::FriendlyNPC) == LevelMap::FriendlyNPC ||
				 ...
				 (*p & LevelMap::DeadBody) == LevelMap::DeadBody || (*p & LevelMap::Item) == LevelMap::Item

				 ) &&
				(*p & LevelMap::BlockWalk) != LevelMap::BlockWalk && (*p & LevelMap::Wall) != LevelMap::Wall &&
	...
	for (UnitAny* pUnit = pRoom1->pUnitFirst; pUnit; pUnit = pUnit->pRoomNext)
	{
		...
		if (pUnit->dwType == 2 && (0 == strcmp(tmp, "Barrel") || 0 == strcmp(tmp, "Door") || 0 == strcmp(tmp, "door") ||
								   0 == strcmp(tmp, "Urn") || 0 == strcmp(tmp, "LargeUrn")))
		{
...
bool LevelMap::IsGap(int x, int y, bool abs) const
{
	...
	if (SpaceIsWalkable(pt, abs)) return false;
	...
	{
		Point pt(i, y);
		if (IsValidPoint(pt, abs)) spaces = (SpaceIsWalkable(pt, abs) ? 0 : spaces + 1);
	}

	if (spaces < gapSize) return true;
...
void LevelMap::FillGaps(void)
{
	...
	for (int i = 0; i < height; i++)
		for (int j = 0; j < width; j++)
			if (IsGap(i, j, false)) SetCollisionData(i, j, LevelMap::Avoid);
...
void LevelMap::ThickenWalls(void)
{
	...
	for (int i = 0; i < height; i++)
	{
		...
		{
			Point pt(i, j);
			if (SpaceIsWalkable(pt, false) || GetMapData(pt, false) == LevelMap::ThickenedWall) continue;
...
void LevelMap::FindRoomLinkageExits(ExitArray& exits, RoomList& added) const
{
	...
	for (Room2* room = level->pRoom2First; room; room = room->pRoom2Next)
	{
		...
		for (DWORD i = 0; i < room->dwRoomsNear; i++)
		{
			...
			{
				// Print("ÿc1d2bsÿc3LevelMap::GetExitsÿc0 adjecent room is out of reach - it's not even touching local
				// room in corner!");
				continue;
			}
			if (overlappingX > 0 && overlappingY > 0)
			{
				// Print("ÿc1d2bsÿc3LevelMap::GetExitsÿc0 WTF local and adjecent rooms are overlapping (they share some
				// points)!!!");
				continue;
			}
...
bool LevelMap::SpaceIsWalkable(const Point& point, bool abs) const
{
	...
	return !(
		SpaceHasFlag(LevelMap::Avoid, point, abs) || SpaceHasFlag(LevelMap::BlockWalk, point, abs) ||
		SpaceHasFlag(LevelMap::BlockPlayer, point, abs) || SpaceHasFlag(LevelMap::NPCCollision, point, abs) ||
		SpaceHasFlag(LevelMap::Object, point, abs) || SpaceHasFlag(LevelMap::ThickenedWall, point, abs));
}
...
bool LevelMap::SpaceIsWalkableForExit(const Point& point, bool abs) const
{
	...
	return !(
		!IsValidPoint(point, abs) || SpaceHasFlag(LevelMap::Avoid, point, abs) ||
...
{
	return !(
		ValueHasFlag(LevelMap::BlockWalk, value) || ValueHasFlag(LevelMap::BlockPlayer, value) ||
