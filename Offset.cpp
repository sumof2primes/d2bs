...
void InstallConditional()
{
	...
	{
		if (Conditional[x].enabled == NULL || *Conditional[x].enabled != TRUE) continue;
		Conditional[x].bOldCode = new BYTE[Conditional[x].dwLen];
		::ReadProcessMemory(
			GetCurrentProcess(), (void*)Conditional[x].dwAddr, Conditional[x].bOldCode, Conditional[x].dwLen, NULL);
...
BOOL WriteBytes(void* pAddr, void* pData, DWORD dwLen)
{
	...
	if (!VirtualProtect(pAddr, dwLen, PAGE_READWRITE, &dwOld)) return FALSE;
