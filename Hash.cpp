...
char* HashString(char* dataIn, ALG_ID algo)
{
	...
	if (!CryptAcquireContext(&provider, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT)) return NULL;
...
char* HashFile(wchar_t* file, ALG_ID algo)
{
	...
	HCRYPTHASH hash;
	DWORD lenOut = 0;
	if (!CryptAcquireContext(&provider, NULL, NULL, PROV_RSA_FULL, 0)) return NULL;
	if (!CryptCreateHash(provider, algo, 0, 0, &hash))
	{
		CryptReleaseContext(provider, 0);
		return NULL;
