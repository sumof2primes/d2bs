...
void InitSettings(void)
{
	wchar_t fname[_MAX_FNAME + _MAX_PATH], scriptPath[_MAX_PATH], defaultStarter[_MAX_FNAME], defaultGame[_MAX_FNAME],
		defaultConsole[_MAX_FNAME], debug[6], quitOnHostile[6], quitOnError[6], maxGameTime[6], gameTimeout[6],
...
bool InitHooks(void)
{
	...
	while (!Vars.bActive)
	{
		...
		{
			if (!ScriptEngine::Startup()) return false;
...
{
	return (
		ClientState() == ClientStateInGame ? Vars.szDefault : ClientState() == ClientStateMenu ? Vars.szStarter : NULL);
}
...
void Reload(void)
{
	if (ScriptEngine::GetCount() > 0) Print(L"\u00FFc2D2BS\u00FFc0 :: Stopping all scripts");
