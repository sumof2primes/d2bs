...
MPQTable BaseStatTable[] = {
	...
	{0x96C9F0, 0x96C9FC, lvltypesTable, "lvltypes", ARRAYSIZE(lvltypesTable), 0xFFFF}, // v - Fixed to fully dump the
																					   // same as d2jsp - TechnoHunter
...
DWORD GetBaseTable(int table, int row)
{
	...
	if (table < sizeof(BaseStatTable))
	{
		...
		if (dwMaxEntriesOffset > 0xFFFF) dwMaxEntriesOffset = DWORD((dwMaxEntriesOffset - 0x400000) + d2CommonOff);
...
bool FillBaseStat(int table, int row, int column, void* result, size_t size)
{
	...
	if (!dwRetValue || column > BaseStatTable[table].wTableSize) return false;

	DWORD dwHelperSize = pTable[column + 1].dwFieldOffset - pTable[column].dwFieldOffset;
	if (dwHelperSize > 4) dwHelperSize = 4;
	switch (pTable[column].eFieldType)
	{
		case FIELDTYPE_DATA_ASCII:
			if (size < pTable[column].dwFieldLength) return false;
			memcpy_s(
				result,
				pTable[column].dwFieldLength,
				(BYTE*)(dwRetValue + pTable[column].dwFieldOffset),
				pTable[column].dwFieldLength);
			break;
		...
		case FIELDTYPE_NAME_TO_WORD_2:
			...
			if (((WORD)result) >= 0xFFFF) *(WORD*)result = (((WORD)result) - 0xFFFF) * -1;
			break;
