...
static UnitAny* GetUnitFromTables(
	UnitHashTable* unitTables,
	DWORD dwTypeLow,
	DWORD dwTypeHigh,
	char* szName,
	DWORD dwClassId,
...
UnitAny* GetUnit(char* szName, DWORD dwClassId, DWORD dwType, DWORD dwMode, DWORD dwUnitId)
{
	...
	if (dwType == 3)
		return GetUnitFromTables(
			p_D2CLIENT_ClientSideUnitHashTables, dwType, dwType, szName, dwClassId, dwType, dwMode, dwUnitId);

	if (dwType >= 0 && dwType <= 5)
		return GetUnitFromTables(
			p_D2CLIENT_ServerSideUnitHashTables, dwType, dwType, szName, dwClassId, dwType, dwMode, dwUnitId);
	else
		return GetUnitFromTables(
			p_D2CLIENT_ServerSideUnitHashTables, 0, 5, szName, dwClassId, dwType, dwMode, dwUnitId);
...
static UnitAny* GetNextUnitFromTables(
	UnitAny* curUnit,
	UnitHashTable* unitTables,
	DWORD dwTypeLow,
	DWORD dwTypeHigh,
...
UnitAny* GetNextUnit(UnitAny* pUnit, char* szName, DWORD dwClassId, DWORD dwType, DWORD dwMode)
{
	...
	if (dwType == 3)
		return GetNextUnitFromTables(
			pUnit, p_D2CLIENT_ClientSideUnitHashTables, dwType, dwType, szName, dwClassId, dwType, dwMode);

	if (dwType >= 0 && dwType <= 5)
		return GetNextUnitFromTables(
			pUnit, p_D2CLIENT_ServerSideUnitHashTables, dwType, dwType, szName, dwClassId, dwType, dwMode);
	else
		return GetNextUnitFromTables(
			pUnit, p_D2CLIENT_ServerSideUnitHashTables, 0, 5, szName, dwClassId, dwType, dwMode);
