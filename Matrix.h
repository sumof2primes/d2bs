...
{
	if (nIndex < 0 || nIndex >= m_cx) return NULL;
...
SIZE CMatrix<TYPE, ARG_TYPE>::ExportData(TYPE** ppBuffer, int cx, int cy) const
{
	...
	if (ppBuffer == NULL || cx <= 0 || cy <= 0 || !IsCreated()) return cz;
...
{
	if (&rMatrix == this) return IsCreated();

	if (!IsCreated()) return FALSE;

	if (rMatrix.GetCX() != m_cx || rMatrix.GetCY() != m_cy)
	{
		if (!rMatrix.Create(m_cx, m_cy)) return FALSE;
	}
...
{
	if (ppSrc == m_ppData) return IsCreated();

	Destroy();
	if (ppSrc == NULL || cx <= 0 || cy <= 0) return FALSE;
