...
public:
	...
	void DumpStats(FILE* f) const
	{
		...
		double realalloctime = (alloctime / freq), realdealloctime = (dealloctime / freq),
			   realctortime = (ctortime / freq), realdtortime = (dtortime / freq),
		...
		fprintf(
			...
			"Time spent allocating: %gms (%f microseconds/alloc), Time spent deallocating: %gms (%f "
			"microseconds/dealloc)\n"
			"Time spent constructing: %gms (%f microseconds/ctor), Time spent destructing: %gms (%f "
			"microseconds/dtor)\n",
			allocs,
			deallocs,
			ctors,
