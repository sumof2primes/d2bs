...
void FlushPrint()
{
	...
	while (!clean.empty())
	{
		...
		if (Vars.bUseGamePrint && ClientState() == ClientStateInGame)
		{
			...
			/*} else if (Vars.bUseGamePrint && ClientState() == ClientStateMenu && findControl(4, (const wchar_t*)NULL,
			   -1, 28, 410, 354, 298)) { while (getline(ss, temp)) SplitLines(temp, Console::MaxWidth() - 100, ' ',
			   lines);
